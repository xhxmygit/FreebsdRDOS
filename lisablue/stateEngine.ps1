...
function RunICTests([XML] $xmlConfig)
{
    ...
    foreach ($vm in $xmlConfig.config.VMs.vm)
    {
        ...
        #
        $xmlElementsToAdd = @("currentTest", "stateTimeStamp", "state", "emailSummary", "jobID", "testCaseResults", "iteration", "ipv4Defined")
...
function DoStartSystem([System.Xml.XmlElement] $vm, [XML] $xmlData)
{
    ...
    CleanMimicArp $xmlData $v.NetworkAdapters
...
function DoSystemStarting([System.Xml.XmlElement] $vm, [XML] $xmlData)
{
    ...
    else
    {
        ...
        # if it was specified.
        #
        if ($vm.ipv4Defined -ne "True")
        {
            LogMsg 9 "Debug: <ipv4> is NULL"
            $ipv4 = GetIPv4 $vm.vmName $vm.hvServer $xmlData
            if (!$ipv4)
            {
                return
            }
            ...
            if ($vm.ipv4 -ne [String] $ipv4)
            {
                LogMsg 5 "Updating VM IP from $($vm.ipv4) to ${ipv4}"
                $vm.ipv4 = [String] $ipv4
            }
        }
...
function DoTestRunning([System.Xml.XmlElement] $vm, [XML] $xmlData)
{
	...
	#if IP may change during test running get IP before check state file
	if ($testData.rebootInRunning)
	{
		$ipv4 = GetIPv4 $vm.vmName $vm.hvServer $xmlData
		if ($ipv4)
		{
...
#
########################################################################
function CleanMimicArp ([xml] $xmlData, [Microsoft.HyperV.PowerShell.VMNetworkAdapter[]] $adapters)
{
    foreach($adapter in $adapters)
    {
        MimicArpClean $xmlData $adapter.MacAddress
    }
}
