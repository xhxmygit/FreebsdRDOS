...
function ShutDownVM([System.Xml.XmlElement] $vm)
{
    ...
    if ($($v.State) -ne "Off")
    {
        if (-not (SendCommandToVM $vm "init 0") )
        {
            ...
            Stop-VM $($vm.vmName) -ComputerName $($vm.hvServer) -AsJob
        }
    }
}
...
#######################################################################
#
# GetIPv4ViaMimicArpServer()
#
#######################################################################
function GetIPv4ViaMimicArpServer([String] $vmName, [String] $server, [xml] $xmlData)
{
    <#
    .Synopsis
        Use mimic arp server to retrieve the VMs IPv4 address.
    .Description
        Use mimic arp server.exe to read the IP address from the VM.
    .Parameter vmName
        Name of the VM to retrieve the IP address from.
    .Parameter server
        Name of the server hosting the VM
    .Example
        GetIPv4ViaMimicArpServer $testVMName $serverName
    #>
    ...
    if (-not $vm)
    {
        Write-Error -Message "GetIPv4ViaMimicArpServer: Unable to get VM ${vmName}" -Category ObjectNotFound -ErrorAction SilentlyContinue
        return $null
    }
    ...
    foreach($adapter in $vm.NetworkAdapters)
    {
        ...
        if($macAddr)
        {
            $ipv4 = MimicArpGet $xmlData $macAddr
            if($ipv4)
            {
                return $ipv4
            }
    ...
    Write-Error -Message "GetIPv4ViaMimicArpServer: Unable to determine IP address from NIC's MAC address" -Category ObjectNotFound -ErrorAction SilentlyContinue
    return $null
}
...
#
#######################################################################
function GetIPv4([String] $vmName, [String] $server, [xml] $xmlData)
{
    <#
    .Synopsis
    ...
    $errMsg = $null
    $methods = @("GetIPv4ViaKVP", "GetIPv4ViaHyperV", "GetIPv4ViaMimicArpServer", "GetIPv4ViaICASerial")
    foreach($method in $methods)
    {
        $addr = & $method $vmName $server $xmlData
        if ($addr)
        {
            LogMsg 5 "INFO : Got VM IPv4 by ${method}: $addr"
            return $addr
        }
        $errMsg += "`n" + $error[0].Exception.Message
    }

    Write-Error -Message ("GetIPv4: Unable to determine IP address for VM ${vmNAme}" + $errmsg) -Category ReadError -ErrorAction SilentlyContinue
    return $null
}
